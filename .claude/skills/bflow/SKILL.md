...
**Papercut**: version files can conflict on merge (hotfix vs. develop, major release vs. develop) — resolve manually, bflow does not auto-resolve. In protected mode resolve on the `finish/*` PR branch, never on the source branch.
