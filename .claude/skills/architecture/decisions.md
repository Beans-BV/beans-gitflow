...
- `run_flow` still takes 10 parameters (`#[allow(clippy::too_many_arguments)]`). Collapsing further would mean a params struct — ceremony beyond the problem, judged not worth it.
- Mainline resolution runs on the `--abort` path too, so a hand-mangled `bflow.branch.main` surfaces there rather than being bypassed. Accepted: the message names the one command that fixes it, and the alternative was a second encoding of the abort rule.
